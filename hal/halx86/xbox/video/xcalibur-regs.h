...
/* Cromwell defined u32 as `unsigned long` here; the compat shim already
 * provides u32 as `unsigned int` (same width on i386), so reuse it. */

/* These register sets are consumed only by the CODE_SEG("INIT") encoder
 * setup in xcalibur.c, so keep them in the discardable INIT section
 * rather than letting const data sit resident in .rdata for the life of
 * the kernel. */
static const u32 Composite_XCal_Vals_PAL[] DATA_SEG("INITDATA") = {
...
static const u32 Composite_XCal_Vals_NTSC[] DATA_SEG("INITDATA") = {
...
static const u32 HDTV_XCal_Vals_480p[] DATA_SEG("INITDATA") = {
