...
*/
/* Ported into nxkrnl HAL via cromwell; original xbox-linux header preserved
 * above.  Its "See the file COPYING" refers to the Linux kernel COPYING,
 * i.e. GPL-2.0 only. */
...
/* Consumed only by the CODE_SEG("INIT") encoder setup; keep it in the
 * discardable INIT section instead of resident .rdata. */
static const unsigned char focus_defaults[0xc4] DATA_SEG("INITDATA") = {
...
int focus_calc_pll_settings(focus_pll_settings *settings, volatile unsigned char *regs);
